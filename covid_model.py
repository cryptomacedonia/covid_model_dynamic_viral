...
def agent_portrayal(agent):
    ...
    if agent.type == "human":
        portrayal["Color"] = 'rgb('+str(agent.age*2)+','+str(agent.age*2)+','+str(agent.age*2)+')'
        portrayal["Layer"] = 0
...
class CovidModel(Model):
    ...
    def generatePeople(self,selectedCount):
        ...
        for line in file:
                ...
                dt = {}
                dt[str(lineSplittedList[0])] = {"male":float(lineSplittedList[1]),"female":float(lineSplittedList[2])}
                prob_of_death_list[str(lineSplittedList[0])] = {"male":float(lineSplittedList[1]),"female":float(lineSplittedList[2])}
        for age_group in self.population_pyramid:
            total = total + int(age_group["male_population"]) + int(age_group["female_population"])
        
        for age_group in self.population_pyramid:
            age_group["male_population"] = int(age_group["male_population"])/total
            age_group["female_population"] = int(age_group["female_population"])/total
        while len(people)<=selectedCount:
             for age_group in self.population_pyramid:
                 if random.randrange(100) < age_group["female_population"]*100:
                     age_random_in_range = random.randrange(int(age_group["age_from"]),int(age_group["age_to"]),1)
                     people.append({"sex":"female","age":age_random_in_range,"death_chance_percentage":float(prob_of_death_list[str(age_random_in_range)]["female"])})
                 if random.randrange(100) < age_group["male_population"]*100:
                     age_random_in_range = random.randrange(int(age_group["age_from"]),int(age_group["age_to"]),1)
                     people.append({"death_chance_percentage":float(prob_of_death_list[str(age_random_in_range)]["male"]), "sex":"male","age":age_random_in_range})
        return people
